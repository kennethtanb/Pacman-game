...
#define GHOST_NUM 4
/* global variables*/
extern bool customMap;
extern const uint32_t GAME_TICK_CD;
extern const uint32_t DEATH_TICK;
extern uint32_t GAME_TICK;
extern ALLEGRO_TIMER* game_tick_timer;
int game_main_Score = 0,game_main_Highscore;
int count = 0,tick_counter=0;
bool game_over = false,PowerBeanEffect = false,winFlag = false;

/* Internal variables*/
static ALLEGRO_TIMER* power_up_timer;
static ALLEGRO_EVENT_QUEUE* power_up_queue;
static ALLEGRO_SAMPLE_ID gameBG;
static const int power_up_duration = 20;
static int beanCur=0;
static Pacman* pman;
...
static void init(void) {
	...
	GAME_TICK = 0;
    stop_bgm(gameBG);
    gameBG = play_bgm(gameMusic, music_volume);
	render_init_screen();
	power_up_timer = al_create_timer(1.0f/ 64 /2);
    power_up_queue = al_create_event_queue();
    al_register_event_source(power_up_queue, al_get_timer_event_source(power_up_timer));
	if (!power_up_timer)
...
static void checkItem(void) {
	...
	switch (basic_map->map[Grid_y][Grid_x])
	{
        case '.':
            ...
            game_main_Score+=10;
            beanCur++;
            if(game_main_Score>game_main_Highscore) game_main_Highscore = game_main_Score;
            break;
        case 'P':
            pacman_eatItem(pman,basic_map->map[Grid_y][Grid_x]);
            basic_map->map[Grid_y][Grid_x]=0;
            PowerBeanEffect = true;
            ...
            //al_wait_for_event(power_up_queue, &ev);
...
static void status_update(void) {
	...
	for (int i = 0; i < GHOST_NUM; i++) {
		...
		if(!cheat_mode && RecAreaOverlap(getDrawArea(pman->objData, GAME_TICK_CD),getDrawArea(ghosts[i]->objData, GAME_TICK_CD))){
            if (ghosts[i]->status == FLEE) ghost_collided(ghosts[i]);
            else{
...
static void update(void) {
	...
	if (game_over && !winFlag) {
    
        ALLEGRO_EVENT_QUEUE *death_queue = NULL;
        ...
        death_queue = al_create_event_queue();
        
        al_register_event_source(death_queue, al_get_timer_event_source(pman->death_anim_counter));
        al_start_timer(pman->death_anim_counter);
        
        while(1){
            ALLEGRO_EVENT ev;
            al_wait_for_event(death_queue, &ev);
            ...
            pacman_death(pman, cF*16);
    
            al_flip_display();
            printf("%d\n",cF);
        }
        game_change_scene(scene_winlost_create());
		
		return;
	}
    if (beanCur >= basic_map->beansNum){
        beanCur = 0;
        game_over = true;
        winFlag = true;
        stop_bgm(gameBG);
    ...
    if (PowerBeanEffect){
        ALLEGRO_EVENT ev;
        al_get_next_event(power_up_queue, &ev);
       // printf("%d\n ",GAME_TICK);
        if(ev.type == ALLEGRO_EVENT_TIMER && ev.timer.source == power_up_timer && GAME_TICK){
            if(++tick_counter>=64){
                if(++count >= power_up_duration){
                    count = 0;
                    PowerBeanEffect = false;
                    for (int i=0;i<GHOST_NUM;i++){
    ...
    //printf("%d %d\n",ghosts[1]->objData.preMove,ghosts[1]->objData.nextTryMove);
	step();
	checkItem();
	status_update();
...
static void draw(void) {
    ...
    char hscore_str[100]={};
    sprintf(hscore_str, "HIGHSCORE: %d",game_main_Highscore);
    al_draw_text(
        menuFont,
        al_map_rgb(255, 255, 255),
        600, 10,
...
static void destroy(void) {
	...
	*/
    ...
    al_destroy_event_queue(power_up_queue);
